...
무료 주소에는 GitHub 계정명이 들어갑니다. 이걸 없애고 `www.kratos.kr` 처럼 만들려면
도메인을 하나 사야 합니다. 확인해 보니 **`kratos.kr` 은 아직 등록되지 않아 구매 가능**합니다.
(`kratos.com` 은 이미 등록되어 있습니다.)

**1) 도메인 구입** — 가비아·후이즈·카페24 중 아무 곳이나. `.kr` 은 연 1.5만~2.2만 원 수준이고
   국내 실명 확인이 필요합니다.
...
```bash
node set-domain.mjs www.kratos.kr
```
